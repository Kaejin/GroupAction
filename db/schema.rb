...
ActiveRecord::Schema.define(version: 20170314120716) do

  # These are extensions that must be enabled in order to support this database
  enable_extension "plpgsql"

  create_table "direct_dontations", force: :cascade do |t|
    t.float    "amount"
    t.integer  "user_id"
    t.integer  "fundraising_event_id"
    t.datetime "created_at",           null: false
    t.datetime "updated_at",           null: false
    t.index ["fundraising_event_id"], name: "index_direct_dontations_on_fundraising_event_id", using: :btree
    t.index ["user_id"], name: "index_direct_dontations_on_user_id", using: :btree
  end
  ...
  add_foreign_key "direct_dontations", "fundraising_events"
  add_foreign_key "direct_dontations", "users"
